...
> **注**：以下为原始论文的描述。TradingAgents-CN 已将双模型架构调整为"分析师模型（一阶段）+ 辩论推理模型（二至四阶段）"，两个模型均为高质量模型，区别在于侧重点不同（低幻觉/数据准确 vs 强逻辑推理），而非"快速 vs 深度"。

为了满足我们框架中任务的多样化复杂性和速度需求，我们根据大语言模型(LLM)的优势战略性地选择它们。分析师模型，如 qwen-plus 和 deepseek-chat，有效处理数据收集和工具调用任务，侧重低幻觉和数字准确。相比之下，辩论推理模型如 qwen-max 在推理密集型任务中表现出色，如决策制定、基于证据的报告写作和数据分析。这些模型利用其架构进行多轮推理，产生逻辑合理、深入的洞察。此外，我们优先考虑具有经过验证的可靠性和可扩展性的模型，以确保在各种市场条件下的最佳性能。我们还为情绪分析等专门任务使用辅助专家模型。

具体来说，所有一阶段分析师节点使用分析师模型以确保数据准确性和低幻觉，而辩论推理模型处理二至四阶段的多轮辩论和决策任务。通过将模型选择与每个阶段的特定要求对齐，我们的框架在数据准确性和推理质量之间实现了平衡，这对有效的交易策略至关重要。
