
export interface PhaseConfig {
  id: number
  name: string
  title: string
  description: string
  agents: string[]
  defaultRounds: number
  minRounds: number
  maxRounds: number
  estimatedTimePerRound: number  // 分钟
  hasDebateRounds?: boolean  // 是否需要辩论轮次设置（默认true，第四阶段为false）
}

export const PHASES: PhaseConfig[] = [
  {
    id: 2,
    name: 'phase2',
    title: '第二阶段 - 双向辩论',
    description: '看涨分析师与看跌分析师进行对抗性辩论，由研究部主管裁决。专业交易员始终执行，不受此开关影响',
    agents: ['bull-researcher', 'bear-researcher', 'research-manager'],
    defaultRounds: 2,
    minRounds: 1,
    maxRounds: 5,
    estimatedTimePerRound: 3
  },
  {
    id: 3,
    name: 'phase3',
    title: '第三阶段 - 风险管理与对冲',
    description: '保守/中性/激进三类风险分析师与风险经理协同评估，制定风控方案',
    agents: ['safe-analyst', 'neutral-analyst', 'risky-analyst', 'risk-manager'],
    defaultRounds: 1,
    minRounds: 0,
    maxRounds: 3,
    estimatedTimePerRound: 3
  },
  {
    id: 4,
    name: 'phase4',
    title: '第四阶段 - 交易决策',
    description: '专业交易员综合分析结果制定交易计划，总结智能体生成最终分析报告（固定执行1次，无需设置轮次）',
    agents: ['trader', '总结智能体'],  // trader 名来自配置；总结智能体为系统内置固定角色，无配置
    defaultRounds: 1,
    minRounds: 1,
    maxRounds: 1,
    estimatedTimePerRound: 2,
    hasDebateRounds: false  // 第四阶段不需要辩论轮次设置
  }
]

// 估算总耗时（分钟）
export function estimateTotalTime(phases: Record<string, { enabled: boolean, debateRounds: number }>): number {
  let total = 5  // 第一阶段基础时间

  PHASES.forEach(phase => {
    const phaseKey = phase.name as keyof typeof phases
    if (phases[phaseKey]?.enabled) {
      // 第四阶段固定执行1次，不需要辩论轮次计算
      if (phase.hasDebateRounds === false) {
        total += phase.estimatedTimePerRound
      } else {
        // 其他阶段：每轮时间 × 辩论轮次
        total += phase.estimatedTimePerRound * phases[phaseKey].debateRounds
      }
    }
  })

  return total
}
