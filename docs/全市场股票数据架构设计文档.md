...
**任何数据源 → 整理成标准格式入库 → 全项目只消费标准数据库。**

这是数据层的核心发展路线，所有贡献（含 PR）必须遵循：

1. **接入新数据源是纯增量工作**：只需为新源编写 Provider / Adapter，把数据规范化成数据库的标准 schema 入库；项目所有消费方（agent 工具、路由、服务）只通过 `DataInterface` 读取标准库，不感知具体数据源的存在
2. **消费层禁止直连数据源**：不得出现"查库不到就现场调用数据源 API 再自己转换格式"的兜底逻辑。正确路径是 `di.read()` → 触发 `di.refresh()` 补数 → 再读；多源回退、熔断、限流全部收敛在 `app/data/` 内部
3. **标准 schema 使用中立字段**：统一使用 `symbol` / `trade_date` 等中立字段，不得携带任何单一数据源的方言（例如把 tushare 的 `ts_code` 作为必填校验字段会导致 akshare 数据天然不合格）
4. **同一 domain 跨源一致性**：不同源入库同一 domain 时，数据形状与主键语义必须一致；主键在 domain 级显式声明，不靠采样记录猜测
5. **连接层可配置**：数据源上游地址覆写等环境差异必须由环境变量 / settings 开关控制且默认关闭，不得在代码中无条件生效
