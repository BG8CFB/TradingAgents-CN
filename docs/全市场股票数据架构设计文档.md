...
| 同步时间 | 市场 × 数据域 | 高级用户可调整 |
| 凭据配置 | 市场级 | `TUSHARE_*_TOKEN`、`FINNHUB_API_KEY`、`ALPHA_VANTAGE_API_KEY` |
...
| `/api/{market}/data/sync/trigger` | POST | 触发指定域同步 |
| `/api/{market}/data/sync/status` | GET | 查询同步状态 |
| `/api/{market}/data/sync/events` | GET | 查询同步事件 |
| `/api/{market}/data/sources/health` | GET | 查询源健康度 |
| `/api/{market}/data/config/priority` | GET/PUT | 获取/更新优先级 |
...
- `connect_status`、`southbound_holding`、`tushare_universe`、`pre_post_market` 等扩展域在代码中已经出现于枚举、调度或集合命名层，但仍未全部形成 `Reader + Repo + Refresh + Router` 的完整闭环；当前策略是对未闭环域显式报错，而不是静默记为成功  
- 因此，本文档更准确的定位是：**当前实现 + 明确演进方向** 的统一设计文档，而不是“所有列出能力均已百分之百落地”的完成态说明

### 维护原则

后续如果继续扩展 `connect_status`、`southbound_holding`、`pre_post_market` 等域，必须同步补齐以下 4 个位置，避免再次出现“枚举已声明但链路未闭环”的问题：
