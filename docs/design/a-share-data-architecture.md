...
| 数据域优先级 | 按市场 × 数据域 × 数据源 | 见能力注册表 | 用户可拖拽调整每个域内各源的顺序 |
| 数据域级源屏蔽 | 按市场 × 数据域 × 数据源 | 全部启用 | 用户可在某个域上**单独禁用**某个源（不影响该源在其他域的使用） |
| Tushare CN Token | **A 股专属** | 空 | **与港股 / 美股 Token 完全独立**（环境变量 `TUSHARE_CN_TOKEN`）；未配置则自动禁用 Tushare（A 股） |
| 自动同步开关 | 按数据域 | 全部开启 | 用户可关闭某个域的自动同步 |
| 同步时间 | 按数据域 | 见调度频率表 | 高级用户可调整同步时间 |

> 关于 Tushare 三市场 Token 独立的统一规范，详见总览文档 `data-platform-overview.md` 第 5.3.4 节。本文档仅描述 A 股市场的 Token（`TUSHARE_CN_TOKEN`），与港股 / 美股 Token 完全独立、配额独立计算（除非用户在三市场使用同一 Token，由 `RateLimiter` 自动按 Token 哈希聚合）。
...
│   ├── 数据源启用/禁用（含按域单独屏蔽）
│   ├── 各域优先级排序（拖拽）
│   ├── Tushare CN Token 配置（A 股专属，独立于港股 / 美股）
│   └── 同步时间调整
│
├── 股票数据查看
│   ├── 按股票查看各域数据
