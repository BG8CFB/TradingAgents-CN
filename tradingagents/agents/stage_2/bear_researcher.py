...
"""
            ...
            for i in range(current_round_index):
                if i < len(rounds):
                    ...
                    if "bull" in round_data:
                        ...
                        messages.append(HumanMessage(content=f"【回顾】这是对手（看涨分析师）在第 {i+1} 轮提出的观点：\n{prev_bull_content}"))
                    
                    # 2. 注入己方之前的观点 (Memory)
                    if "bear" in round_data:
                        prev_bear_content = round_data["bear"]
                        # 使用 AIMessage 表示这是"我"之前说的话
                        messages.append(AIMessage(content=f"【回顾】这是我在第 {i+1} 轮提出的风险警示：\n{prev_bear_content}"))
        ...
        if current_round_index == 0:
            # Round 0: 初始立论
            instruction = f"""请基于上述提供的所有分析报告，撰写【看跌分析报告】的**第一部分：核心风险警示**。

要求：
1. 重点强调市场饱和、财务不稳定、竞争劣势或宏观经济威胁。
2. 引用报告中的具体负面数据作为支撑。
3. **不要**写反驳内容（因为还没看到看涨观点），只专注于构建风险提示。
...
"""
            ...
            if prev_round_idx < len(rounds) and "bull" in rounds[prev_round_idx]:
                latest_opponent_arg = rounds[prev_round_idx]["bull"]
                # 再次强调这是需要反驳的目标
                messages.append(HumanMessage(content=f"🔥 **当前任务目标**：请针对对手（看涨分析师）刚刚提出的上述最新观点（第 {current_round_index} 轮）进行重点批判！"))
            else:
                logger.warning(f"🐻 [WARNING] 未找到 Round {prev_round_idx} 的看涨观点，将进行通用质疑。")
            
            instruction = f"""请撰写【看跌分析报告】的**第 {current_round_index + 1} 部分：深度质疑与反驳**。

要求：
1. 紧扣对手刚刚提出的增长潜力或优势进行逐点批判，揭示其潜在风险或不可持续性。
2. 结合你自己在第一轮建立的风险框架（已在上下文中）和基础报告数据，证明对手的假设可能过于乐观。
3. 强调不确定性和下行风险。
4. 如果这是最后一轮（第 {max_rounds} 轮），请在最后包含一个强有力的**结语**，劝诫投资者保持谨慎。
...
"""
        ...
        # 累积到最终报告
        section_title = f"## 第 {current_round_index + 1} 部分：核心风险警示" if current_round_index == 0 else f"## 第 {current_round_index + 1} 部分：针对多方观点的质疑与反驳"
        ...
        # 保持对旧 state 字段的兼容
        argument_prefix = f"Bear Analyst (Round {current_round_index}):"
