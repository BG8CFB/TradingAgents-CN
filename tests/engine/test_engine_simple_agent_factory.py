...
注意：配置加载/查找方法已合并到 DynamicAnalystFactory，
SimpleAgentFactory 仅保留 create_analysts() 工厂方法。
"""
...
class TestLoadConfig:
    def test_loads_from_explicit_path(self, sample_yaml_config):
        config = DynamicAnalystFactory.load_config(sample_yaml_config)
        assert "customModes" in config
        assert len(config["customModes"]) == 2

    def test_returns_empty_for_missing_file(self):
        config = DynamicAnalystFactory.load_config("/nonexistent/path/config.yaml")
        assert config == {}
...
class TestGetAgentConfig:
    def test_find_by_slug(self, sample_yaml_config):
        agent = DynamicAnalystFactory.get_agent_config("market-analyst", sample_yaml_config)
        assert agent is not None
        assert agent["slug"] == "market-analyst"

    def test_find_by_internal_key(self, sample_yaml_config):
        agent = DynamicAnalystFactory.get_agent_config("market", sample_yaml_config)
        assert agent is not None
        assert agent["slug"] == "market-analyst"

    def test_find_by_chinese_name(self, sample_yaml_config):
        agent = DynamicAnalystFactory.get_agent_config("新闻分析师", sample_yaml_config)
        assert agent is not None
        assert agent["slug"] == "news-analyst"

    def test_returns_none_for_unknown(self, sample_yaml_config):
        agent = DynamicAnalystFactory.get_agent_config("nonexistent-agent", sample_yaml_config)
        assert agent is None


class TestGetAllAgents:
    def test_merges_custom_modes_and_agents(self, sample_yaml_config):
        agents = DynamicAnalystFactory.get_all_agents(sample_yaml_config)
    ...
    def test_returns_empty_list_for_empty_config(self, tmp_path):
        ...
        agents = DynamicAnalystFactory.get_all_agents(str(config_path))
        assert agents == []


class TestBuildProgressMap:
    def test_map_contains_fixed_entries(self, sample_yaml_config):
        pm = DynamicAnalystFactory.build_progress_map(config_path=sample_yaml_config)
        assert "🐂 看涨研究员" in pm
        assert "🐻 看跌研究员" in pm
        assert "👔 研究经理" in pm
        assert "📊 生成报告" in pm
...
class TestGetAnalystIcon:
    def test_news_icon(self):
        assert DynamicAnalystFactory._get_analyst_icon("news-analyst") == "📰"

    def test_social_icon(self):
        assert DynamicAnalystFactory._get_analyst_icon("social-analyst") == "💬"

    def test_fundamental_icon(self):
        assert DynamicAnalystFactory._get_analyst_icon("fundamentals-analyst") == "💼"

    def test_china_icon(self):
        assert DynamicAnalystFactory._get_analyst_icon("china-analyst") == "🇨🇳"

    def test_capital_icon(self):
        assert DynamicAnalystFactory._get_analyst_icon("capital-analyst") == "💸"

    def test_market_icon(self):
        assert DynamicAnalystFactory._get_analyst_icon("market-analyst") == "📊"

    def test_default_icon(self):
        assert DynamicAnalystFactory._get_analyst_icon("custom-analyst") == "🤖"

    def test_name_based_icon(self):
        assert DynamicAnalystFactory._get_analyst_icon("x", "新闻分析") == "📰"
        assert DynamicAnalystFactory._get_analyst_icon("x", "基本面分析") == "💼"
