...
# 数据源描述映射
SOURCE_DESCRIPTIONS = {
    "yfinance_us": "Yahoo Finance美股数据源，提供基础信息和行情数据（免费，无需API Key）",
    "finnhub_us": "Finnhub美股数据源，提供实时行情和新闻数据（需API Key，免费版有频率限制）",
}

# 默认数据源优先级（回退用）
_DEFAULT_PRIORITY = {"yfinance_us": 10, "finnhub_us": 5}

# adapter 名称 -> 数据库中 data_source_name 的映射
_ADAPTER_TO_DB_NAME = {
    "yfinance_us": "yahoo_finance",
    "finnhub_us": "finnhub",
}
...
class USBatchWarmRequest(BaseModel):
    symbols: List[str] = Field(..., description="美股代码列表")
    force: bool = Field(False, description="是否强制刷新")


class USSourceTestRequest(BaseModel):
...
async def get_us_sources_status():
    ...
    try:
        ...
        priority_map = get_source_priority("US")
        adapters = [
...
async def test_us_sources(
    request: USSourceTestRequest,
    ...
    try:
        ...
        for name, factory in adapters:
            try:
                ...
                if available:
                    try:
                        info = await adapter.provider.get_stock_basic_info("AAPL")
                        if info:
                            stock_name = info.get("name", "") or info.get("longName", "") or info.get("shortName", "")
                            message = f"连接成功，成功获取Apple({stock_name})数据"
                        else:
...
async def list_us_cached_stocks(
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    current_user: dict = Depends(get_current_user),
):
    """获取美股已缓存股票列表（分页，按更新时间倒序）"""
    try:
        from app.worker.us import get_us_cache_service
        service = get_us_cache_service()
...
async def warm_us_cache(
    ...
    try:
        ...
        if result["info_success"]:
            msg = f"美股 {request.symbol} 缓存预热成功（基础信息+{result['quotes_count']}条行情，来源: {result['source']}）"
            return ok(data=result, message=msg)
        else:
            return fail(message=f"美股 {request.symbol} 缓存预热失败", data=result)
    except Exception as e:
...
async def batch_warm_us_cache(
    request: USBatchWarmRequest,
    current_user: dict = Depends(get_current_user),
):
    """批量预热美股缓存（后台执行）"""
    try:
        ...
        logger.error(f"美股批量预热失败: {e}")
        return fail(message=str(e))


@router.get("/cache/warm/status")
async def get_us_warm_status(
    current_user: dict = Depends(get_current_user),
):
    """查询美股批量预热进度"""
    try:
...
async def get_us_recommendations():
    ...
    try:
        ...
        for i, src in enumerate(sources):
            ...
            if i == 0:
                primary_source = {
                    "name": display_name,
                    "priority": priority,
                    "reason": "当前优先级最高的美股数据源" if src == "yfinance" else "当前配置的首选美股数据源",
                }
            else:
                fallback_sources.append({"name": display_name, "priority": priority})

        suggestions = [
            "美股数据采用按需缓存模式（24小时有效期），不会自动定时同步",
            "建议在分析前先预热自选股缓存，确保数据新鲜",
            "YFinance 为免费数据源，无需 API Key，推荐优先使用",
            "Finnhub 需要注册获取 API Key（免费版有 60次/分钟限制），但数据更及时",
            "美股盘前盘后数据可能存在延迟，以正式交易时段数据为准",
            "缓存过期后会自动清理，无需手动维护",
        ]

        if "yfinance" not in sources:
            warnings.append("未启用 YFinance 数据源，建议启用以获得免费的美股数据")

        if "finnhub" in sources:
            import os
            if not os.getenv("FINNHUB_API_KEY"):
                warnings.append("已启用 Finnhub 但未配置 FINNHUB_API_KEY 环境变量，该数据源可能不可用")

        if not sources:
            warnings.append("未配置任何美股数据源，请先在系统设置中配置")
        ...
        return ok(data={
            ...
            "warnings": warnings,
            "env_config": {
                "description": "YFinance 无需配置；Finnhub 需设置 FINNHUB_API_KEY 环境变量",
                "example": "FINNHUB_API_KEY=your_api_key_here",
            },
        })
    except Exception as e:
        logger.error(f"获取美股使用建议失败: {e}")
