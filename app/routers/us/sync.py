...
async def refresh_us_stock(symbol: str, req: RefreshRequest, user=Depends(get_current_user)):
    """按需刷新指定股票的数据"""
    try:
...
async def get_refresh_status(symbol: str):
    ...
    for domain in domains:
        try:
            ...
            logger.debug(f"获取US数据域 {domain} 新鲜度失败: {e}")
...
async def trigger_us_sync(req: SyncTriggerRequest, user=Depends(get_current_user)):
    di = DataInterface.get_instance()
...
async def trigger_sync_by_domain(domain: str, request: SyncTriggerRequest, user=Depends(get_current_user)):
    ...
    try:
        ...
        if engine:
            job_id = await engine.trigger_job("US", domain)
