...
def _build_registry() -> List[BuiltinToolSpec]:
    ...
    return [
        ...
        BuiltinToolSpec(
            tool_id="search_stock_info",
            display_name="股票信息搜索",
            domains=[],
            markets=["CN", "HK", "US"],
            fn=_lazy_import(f"{_M}.web_search", "search_stock_info"),
            inject_args={"stock_code": "ticker"},
            description="在权威财经网站搜索特定股票的信息，数据库无数据时自动使用。支持: news/financial/dragon_tiger/margin/block_trade/northbound/chip/sentiment/analyst/dividend/pledge/unlock/announcement",
        ),
        BuiltinToolSpec(
