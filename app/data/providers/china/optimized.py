...
# 导入 MongoDB 缓存适配器
from app.data.cache.mongodb_cache_adapter import get_mongodb_cache_adapter, get_stock_data_with_fallback, get_financial_data_with_fallback
...
"""
    ...
    def _get_stock_basic_info_only(self, symbol: str) -> str:
        ...
        try:
            ...
            try:
                from app.engine.config.runtime_settings import use_app_cache_enabled
                if use_app_cache_enabled(False):
                    from app.data.cache.app_adapter import get_market_quote_dataframe
    ...
    def _generate_fundamentals_report(self, symbol: str, stock_data: str, analysis_modules: str = "standard") -> str:
        ...
        try:
            if (current_price == "N/A" or change_pct == "N/A" or volume == "N/A"):
                from app.engine.config.runtime_settings import use_app_cache_enabled  # type: ignore
                if use_app_cache_enabled(False):
                    from app.data.cache.app_adapter import get_market_quote_dataframe
                    df_q = get_market_quote_dataframe(symbol)
                    if df_q is not None and not df_q.empty:
...
"""
    ...
    def _get_industry_info(self, symbol: str) -> dict:
        ...
        # 首先尝试从数据库获取真实的行业信息
        try:
            from app.data.cache.app_adapter import get_basics_from_cache
