...
复用现有 BasicsSyncService 的批量写入逻辑。
"""
...
class TushareBasicInfoSync:
    """Tushare 基础信息同步编排"""

    def __init__(self, adapter):
        """
        Args:
            adapter: TushareAdapter 实例
        """
    ...
    async def sync_all(self, force: bool = False, job_id: str = None) -> Dict[str, Any]:
        ...
        if raw_df is None or raw_df.empty:
            logger.error("Tushare 返回空股票列表")
            stats["error"] = "Tushare returned empty stock list"
            return stats

        stats["total"] = len(raw_df)
        logger.info(f"Tushare 基础信息同步开始: {stats['total']} 只股票")
        ...
        for schema in schemas:
            ...
            ops.append(
                UpdateOne(
                    {"symbol": doc["symbol"], "data_source": "tushare"},
                    {"$set": doc},
                    upsert=True,
                )
            )
        ...
        logger.info(f"Tushare 基础信息同步完成: {success}/{stats['total']}")
        return stats

    @staticmethod
    def _bulk_write(collection, ops: List[UpdateOne]) -> int:
        """同步执行批量写入"""
        try:
