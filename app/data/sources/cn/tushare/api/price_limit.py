...
async def fetch_price_limit(
    ...
    try:
        df = await asyncio.to_thread(conn.api.stk_limit, **kwargs)
    ...
    if is_empty_result(df):
        logger.debug(f"Tushare 涨跌停价格为空: {kwargs}")
        raise DataNotFoundError("tushare", _DOMAIN, f"{kwargs} 无数据")

    logger.info(f"Tushare 涨跌停价格: {len(df)} 条")
    return df
