...
class BaoStockBasicInfoSync:
    ...
    async def sync_all(self, force: bool = False, job_id: str = None) -> Dict[str, Any]:
        ...
        if raw_df is None or raw_df.empty:
            ...
            stats["error"] = "BaoStock returned empty stock list"
            return stats

        stats["total"] = len(raw_df)
        logger.info(f"BaoStock 基础信息同步开始: {stats['total']} 只股票")
        ...
        for schema in schemas:
            ...
            ops.append(
                UpdateOne(
                    {"symbol": doc["symbol"], "data_source": "baostock"},
                    {"$set": doc},
                    upsert=True,
                )
            )
        ...
        logger.info(f"BaoStock 基础信息同步完成: {success}/{stats['total']}")
        return stats
