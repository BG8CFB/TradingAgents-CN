    ...
    async def warm_stock_info(self, symbol: str) -> bool:
        ...
        try:
            ...
            db[collection_name].update_one(
                {"symbol": doc["symbol"], "data_source": "finnhub"},
                {"$set": doc},
                upsert=True,
            )
            logger.info(f"✅ [美股/Finnhub] 基础信息预热成功: {symbol}")
            return True
        except Exception as e:
            logger.error(f"❌ [美股/Finnhub] 基础信息预热失败: {symbol} - {e}")
            return False
    ...
    async def warm_daily_quotes(
        ...
        try:
            ...
            if ops:
                ...
                logger.info(f"✅ [美股/Finnhub] 行情预热: {symbol} {saved} 条")
                return saved
            return 0
        except Exception as e:
            logger.error(f"❌ [美股/Finnhub] 行情预热失败: {symbol} - {e}")
            return 0
