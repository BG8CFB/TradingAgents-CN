...
class YFinanceHKOrchestrator:
    ...
    async def warm_stock_info(self, symbol: str) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ [港股/YFinance] 基础信息预热成功: {symbol}")
            return True
        except Exception as e:
            logger.error(f"❌ [港股/YFinance] 基础信息预热失败: {symbol} - {e}")
            return False
    ...
    async def warm_daily_quotes(
        ...
        try:
            ...
            if ops:
                ...
                logger.info(f"✅ [港股/YFinance] 行情预热: {symbol} {saved} 条")
                return saved
            return 0
        except Exception as e:
            logger.error(f"❌ [港股/YFinance] 行情预热失败: {symbol} - {e}")
            return 0
