"""筛选查询服务 — 四阶段跨集合聚合查询（数据层内实现）。

从 app/services/database_screening_service.py 迁入，管道形状保持不变；
统一使用 get_motor_db + get_collection_name 访问标准集合，
消费方通过 DataInterface.screen() / DatabaseScreeningService 薄壳间接调用。
...
class ScreeningQueryService:
    ...
    async def screen_stocks(
        ...
        """四阶段跨集合筛选，返回 (page_items, total_count)。"""
        try:
            db = get_motor_db()
            if not source:
            ...
            final_symbols = (
                matched_symbols if matched_symbols is not None else list(basic_map.keys())
            )
            ...
            indicators_map = await self._fetch_latest_indicators(
                indicator_coll, final_symbols, source
            )
    ...
    async def screen(
        ...
        """单阶段通用筛选查询（DataInterface.screen 的底层实现）。

        Args:
            market: 市场 (CN/HK/US)
            stage: 数据域（basic_info/daily_quotes/market_quotes/...，
                   即 get_collection_name 支持的 domain）
            filters: MongoDB 查询过滤
            projection: 投影（默认排除 _id）
            sort: [(field, direction)] 排序
            skip/limit: 分页
        """
        db = get_motor_db()
        coll = db[get_collection_name(stage, market)]

        proj = dict(projection) if projection else {"_id": 0}
        if "_id" not in proj:
            proj["_id"] = 0
...
# 全局服务实例
_screening_query_service: Optional[ScreeningQueryService] = None


def get_screening_query_service() -> ScreeningQueryService:
    global _screening_query_service
    if _screening_query_service is None:
        _screening_query_service = ScreeningQueryService()
    return _screening_query_service
