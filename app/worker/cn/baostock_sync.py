...
class BaoStockSyncService(BaseSyncService):
    ...
    async def _update_stock_basic_info(self, basic_info: Dict[str, Any]):
        ...
        try:
            ...
            await collection.update_one(
                {"code": basic_info["code"], "data_source": "baostock"},
                {"$set": basic_info},
                upsert=True
            )
    ...
    async def sync_daily_quotes(self, batch_size: int = 50) -> BaoStockSyncStats:
        ...
        try:
            ...
            # 从数据库获取股票列表
            collection = self.db.stock_basic_info
            cursor = collection.find({"data_source": "baostock"}, {"symbol": 1})
            stock_codes = [doc["symbol"] async for doc in cursor]
        ...
        for code in code_batch:
            try:
                ...
                if incremental:
                    ...
                    # 固定天数同步
                    start_date = format_date_short(now_config_tz() - timedelta(days=days))
        ...
        try:
            ...
            if self.db is not None:
                ...
                await collection.update_one(
                    {"code": code},
                    {"$set": {
                        "historical_data_updated": now_config_tz(),
    ...
    async def check_service_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "last_check": format_iso(now_config_tz())
            }
...
def _get_baostock_lock():
    """懒初始化 asyncio.Lock，绑定到当前事件循环"""
    global _baostock_lock
    try:
        loop = asyncio.get_running_loop()
    except RuntimeError:
        loop = None
    if _baostock_lock is None or (loop is not None and _baostock_lock._loop is not loop):
        _baostock_lock = asyncio.Lock()
    return _baostock_lock


async def get_baostock_sync_service() -> BaoStockSyncService:
    """获取 BaoStock 同步服务实例（单例模式，带并发锁保护）"""
    global _baostock_sync_service
    if _baostock_sync_service is not None:
        return _baostock_sync_service
    async with _get_baostock_lock():
