...
class USCacheService:
    """美股按需缓存服务"""

    def __init__(self):
        self._basic_info_collection = get_collection_name("US", "basic_info")
        self._daily_quotes_collection = get_collection_name("US", "daily_quotes")
    ...
    async def get_stock_info(self, stock_code: str) -> Optional[Dict[str, Any]]:
        ...
        for orch_info in self._get_orchestrators():
            try:
                ...
                logger.warning(f"⚠️ [美股缓存] {stock_code} 预热失败: {e}")
                continue

        logger.warning(f"⚠️ [美股缓存] {stock_code} 所有数据源均失败")
        return None
    ...
    async def refresh_cache(self, stock_code: str) -> Optional[Dict[str, Any]]:
        ...
        for orch_info in self._get_orchestrators():
            try:
                ...
                logger.warning(f"⚠️ [美股缓存] 刷新 {stock_code} 失败: {e}")
            ...
            try:
                ...
                if info_ok:
                    try:
                        ...
                        logger.warning(f"⚠️ [美股缓存] {stock_code} 行情预热失败（基础信息已缓存）: {e}")
                        quotes_count = 0
                    used_source = orch_info[0].split('.')[-2].replace('_us', '').upper()
                    break
            except Exception as e:
                logger.warning(f"⚠️ [美股缓存] 预热 {stock_code} 失败: {e}")
                continue
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "available_sources": get_enabled_sources("US"),
