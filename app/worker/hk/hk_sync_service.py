...
class HKSyncService:
    ...
    def __init__(self):
        ...
        self.batch_size = settings.HK_SYNC_BATCH_SIZE
        self.rate_limit_delay = settings.HK_SYNC_RATE_LIMIT_DELAY
    ...
    async def sync_stock_basic_info(self, force_update: bool = False) -> Dict[str, Any]:
        """同步港股基础信息 — 通过 DataInterface 刷新每只股票"""
        ...
        for idx, symbol in enumerate(symbols, 1):
            ...
            try:
                ...
                logger.debug(f"港股 {symbol} 同步基础信息失败: {e}")
                stats["error_count"] += 1
    ...
    async def sync_daily_quotes(
        ...
        """同步港股日线行情 — 通过 DataInterface 刷新每只股票"""
        await self._ensure_initialized()
        ...
        for idx, symbol in enumerate(symbols, 1):
            ...
            # 如果增量模式下 actual_start > end_date，说明数据已最新
            if incremental and actual_start > end_date:
                stats["success_count"] += 1
            else:
                try:
                    refresh_result = await di.refresh(
                        "HK", symbol, domains=["daily_quotes"],
                    ...
                    logger.debug(f"港股 {symbol} 同步行情失败: {e}")
                    stats["error_count"] += 1
    ...
    async def run_status_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"港股状态检查查询数据库失败: {e}")

        available_sources = []
        try:
            health_list = await di.get_source_health("HK")
            available_sources = [h.get("source", "") for h in health_list if h.get("healthy")]
        except Exception as e:
            logger.warning(f"获取港股数据源健康状态失败: {e}")
